...
const clearAllCards = () => {
    // clear globalStore and update localStorage
    globalStore = [];
    updateLocalStorage();
...
// add event listener to the "Clear Page" link
document.getElementById('clearPageLink').addEventListener('click', clearAllCards);
...
const searchItems = () => {
	...
	const filteredItems = globalStore.filter((blogObject) =>
	  blogObject.blogTitle.toLowerCase().includes(searchInput) || blogObject.blogType.toLowerCase().includes(searchInput)
	);
  ...
  const displayFilteredItems = (filteredItems) => {
	// Clear existing cards
	blogContainer.innerHTML = '';
  
	// Display filtered cards
	filteredItems.forEach((blogObject) => {
	  const createNewBlog = newCard(blogObject);
	  blogContainer.insertAdjacentHTML("beforeend", createNewBlog);
	});
  };
